...
| 8 | Sygnał awarii | 🟨 **18–19.09 w toku, wzięty przed 6 i 7 decyzją Gracjana.** Notatka [[Notatka-2026-09-18-sygnal-awarii]] zatwierdzona w całości: zewnętrzny stróż healthchecks.io (plan darmowy), „dowód sukcesu” zamiast szukania złych słów (3 × `stan: zapisane`, `Odebrano`, 2 × `S3: wysłano`, zero `Traceback`), Producent pisze błędy do `errors.txt`, adres zgłoszenia tylko w `crontab`, skrypt kontrolny o 18:30 polskiego z 30 min zapasu, strefa `Europe/Warsaw`. Na laptopie: `kod/control.py` z funkcjami `sprawdz_blok` i `wytnij_blok`, `kod/test_control.py` — 11 testów na prawdziwym bloku z 18.09, wszystkie przechodzą. Znalezione dwie pułapki: własne komunikaty skryptu kontrolnego zawierają szukane słowa (odsiew linii `Kontrola:`, jest test) oraz polskie litery w zgłoszeniu padną na EC2 (`urllib3==1.26.20` → latin-1), choć przejdą na laptopie. **19.09 (commit `b28e3de`) napisane:** część główna skryptu (odczyt logu, zgłoszenie do stróża, linia `Kontrola:`) i zmiana logu w Producencie (`basicConfig` bez `filename`, błędy idą do `errors.txt`). Nie sprawdzone, czy część główna była uruchamiana ani czy testy po tym commicie przechodzą; nie wdrożone — w logach z 19 i 20.09 brak linii `Kontrola:`, a `git log` na EC2 20.09 nie sprawdzany. Brakuje: wdrożenia na EC2, testów z mailem, biegu z `cron` o 18:30, potwierdzenia zadania u stróża |
| 6, 7, 10 | reszta | ⬜ |
