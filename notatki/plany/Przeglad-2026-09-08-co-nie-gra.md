...
| 6 | Wyłączenie lokalnego Harmonogramu, `silver/` i `gold/` poza gitem | 🟨 **21.09 wdrożone na laptopie i EC2, sprawdzian 22.09 o 18:10.** Notatka [[Notatka-2026-09-21-jedno-miejsce-liczenia]] zatwierdzona (`.gitkeep`, zadanie `Disabled`, `pipeline.bat` zostaje, historia gita nietknięta). Commit `6af7b48` (`6 files changed, 4 insertions(+), 4656 deletions(-)`), próba na klonie testowym zgodna, EC2 na `6af7b48` z folderami z samym `.gitkeep`. Brakuje biegu o 18:10 na odtworzonych folderach i przepięcia wykresów, `ranking.py`, `test_plikow.py` i Power BI na Athenę |
| 7, 10 | Test prawdziwej drogi; dokumentacja | ⬜ test prawdziwej drogi jest następny w kolejce (po sprawdzianie punktu 6); dokumentacja w tle |
