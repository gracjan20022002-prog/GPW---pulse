...
| 3c | Konsument: zakładka tylko po udanym zapisie do S3 | ✅ **17.09 naprawione i wdrożone na EC2.** Wada z podejrzenia 14.09 potwierdzona w kodzie `kafka-python 3.0.11` i na żywo (zakładka 2348 → 2351 przy pustym `live/`), naprawiona przez `enable_auto_commit=False` + `consumer.close()`. Testy po zmianie: zapis odcięty → `2348 2351 3`; bieg zwykły → `2351 2351 0`. Notatka: [[Notatka-2026-09-17-zakladka-przed-zapisem]]. **18.09 pierwszy bieg z `cron` na nowym Konsumencie:** blok 28 linii od linii 583 (`16:00:02` UTC), `Odebrano 3 wiadomości`, 2 × `(2325, 3)` — 2325, nie 2328, czyli trzy powtórki z testu B odsiane — 2 × `S3: wysłano`, bez `Traceback`. **Nieodczytane:** zakładka (przewidywana `2354 2354 0`), `wc -l`, `COUNT(*)` w Athenie (przewidywane 2325). Ograniczenie świadome: `close()` za `put_object`, przy awarii się nie wykonuje. Brakuje głośnej awarii (krok 8) |
| 8 | Sygnał awarii | 🟨 **18.09 w toku, wzięty przed 6 i 7 decyzją Gracjana.** Notatka [[Notatka-2026-09-18-sygnal-awarii]] zatwierdzona w całości: zewnętrzny stróż healthchecks.io (plan darmowy), „dowód sukcesu” zamiast szukania złych słów (3 × `stan: zapisane`, `Odebrano`, 2 × `S3: wysłano`, zero `Traceback`), Producent pisze błędy do `errors.txt`, adres zgłoszenia tylko w `crontab`, skrypt kontrolny o 18:30 polskiego z 30 min zapasu, strefa `Europe/Warsaw`. Na laptopie: `kod/control.py` z funkcjami `sprawdz_blok` i `wytnij_blok`, `kod/test_control.py` — 11 testów na prawdziwym bloku z 18.09, wszystkie przechodzą. Znalezione dwie pułapki: własne komunikaty skryptu kontrolnego zawierają szukane słowa (odsiew linii `Kontrola:`, jest test) oraz polskie litery w zgłoszeniu padną na EC2 (`urllib3==1.26.20` → latin-1), choć przejdą na laptopie. Brakuje: części głównej skryptu, zmiany logu w Producencie, wdrożenia na EC2, testów z mailem, biegu z `cron` |
| 6, 7, 10 | reszta | ⬜ |
