...
- 🟠 **Dwie maszyny liczą to samo i obie wersje trafiają do gita.** **21.09 wdrożone: Harmonogram wyłączony, `silver/` i `gold/` poza gitem, sprawdzian 22.09 (patrz tabela stanu).**
...
| 4c | `CRON_TZ` i godziny biegów | ✅ **13.09 wdrożone, sprawdzone biegami 13.09 i 14.09** (szczegóły w 2.10). Brakuje biegu po zmianie czasu — **25.10**, niedziela (poprawny pokaże `17:00:0X`; sprostowane 18.09, wcześniej wpisane 26.10, a to dopiero pierwszy taki bieg w dzień giełdowy) — i głośnej awarii (krok 8) |
...
| 5 | Wynik Golda do S3 i Atheny | ✅ **17.09 wdrożone na EC2 i sprawdzone biegiem `cron`:** `git pull` do `070b478`, `GOLD_DO_S3=1` w `crontab` (`REPLACE` 14:59:47 UTC, `RELOAD` 15:00:01 UTC), 2 × `S3: wysłano`, `COUNT(*)` na `gold_dane_dzienne` **2322** zamiast 2316 — plik policzony przez `cron` sam doszedł do Atheny. 18.09 znów 2 × `S3: wysłano`. Brakuje głośnej awarii (krok 8). **Historia:** 🟨 15.09 w toku. Notatka [[Notatka-2026-09-15-wynik-golda-do-s3]] zatwierdzona, pięć decyzji zgodnie z rekomendacją (nadpisywać, CSV, przełącznik `GOLD_DO_S3` tylko na EC2, tabele ręcznie SQL, bez kolumny z godziną). Uprawnienia sprawdzone przed kodem: rola EC2 ma `AmazonS3FullAccess` i `AmazonAthenaFullAccess`, brak polityk wpisanych w rolę i polityki bucketa, `gold/` puste. Przełącznik napisany, sprawdzony lokalnie w obu drogach; pierwsza wysyłka z laptopa (jednorazowy wyjątek od decyzji 3) co do bajta: 157240 + 365 = 157605. **16.09 tabele w Athenie ✅:** `gold_dane_dzienne` i `gold_ranking_spolek` w bazie `gpw-tracker_db`, założone ręcznie, sprawdzone `COUNT(*)` 2316 i 3 oraz pustą komórką (`NULL`, 3 wiersze). Brakuje: wdrożenia na EC2 ze zmienną w `crontab`, głośnej awarii |
| 3c | Konsument: zakładka tylko po udanym zapisie do S3 | ✅ **17.09 naprawione i wdrożone na EC2.** Wada z podejrzenia 14.09 potwierdzona w kodzie `kafka-python 3.0.11` i na żywo (zakładka 2348 → 2351 przy pustym `live/`), naprawiona przez `enable_auto_commit=False` + `consumer.close()`. Testy po zmianie: zapis odcięty → `2348 2351 3`; bieg zwykły → `2351 2351 0`. Notatka: [[Notatka-2026-09-17-zakladka-przed-zapisem]]. **18.09 pierwszy bieg z `cron` na nowym Konsumencie:** blok 28 linii od linii 583 (`16:00:02` UTC), `Odebrano 3 wiadomości`, 2 × `(2325, 3)` — 2325, nie 2328, czyli trzy powtórki z testu B odsiane — 2 × `S3: wysłano`, bez `Traceback`. **Trzy odczyty domknięte 20.09, wszystkie zgodne z przewidywaniem:** zakładka `2354 2354 0`, pliki spółek 775 × 3, `COUNT(*)` 2325 (po spółce 775 dni od 2023-08-14 do 2026-09-18). W weekendowych biegach 19 i 20.09 zakładka się nie ruszyła (`Odebrano 0`). Ograniczenie świadome: `close()` za `put_object`, przy awarii się nie wykonuje. Brakuje głośnej awarii (krok 8) |
| 8 | Sygnał awarii | ✅ **21.09, z zastrzeżeniami.** Notatka [[Notatka-2026-09-18-sygnal-awarii]] zatwierdzona; `kod/control.py` i 11 testów; 19.09 kod na EC2 (`b28e3de`) i testy 2–3 z prawdziwym stróżem (awaria wymuszona: `Failure`, mail z powodem, 2351 bajtów co do bajta, polskie litery bez krzaków); **21.09** linia `30 18` w `crontab` (16:53 polskiego), test ciszy (mail `DOWN` 19.09 19:00:00 +0200, strefa polska) i pierwszy bieg z `cron` o 18:30:02 zgodny z przewidywaniem (start w linii 663, `wc -l` 691, `Kontrola: OK`, stróż `#3 OK`, `down → up`, mail `UP`). Warunki (a)–(d) spełnione jednym biegiem, (e) opis w CLAUDE.md i dzienniku z 21.09. **Zastrzeżenia:** jeden bieg, nie tydzień; głośna awaria pokazana na dwóch testach wymuszonych, nie na prawdziwej awarii potoku; nie łapie złych danych przy udanym biegu ani braku świecy w dzień giełdowy; strefa dopiero 25.10 |
| 6 | Wyłączenie lokalnego Harmonogramu, `silver/` i `gold/` poza gitem | ✅ **21.09 wdrożone, 22.09 sprawdzian zamknięty.** Notatka [[Notatka-2026-09-21-jedno-miejsce-liczenia]] zatwierdzona (`.gitkeep`, zadanie `Disabled`, `pipeline.bat` zostaje, historia gita nietknięta). Commit `6af7b48` (`6 files changed, 4 insertions(+), 4656 deletions(-)`), próba na klonie testowym zgodna, EC2 na `6af7b48` z folderami z samym `.gitkeep`. **22.09:** pierwszy bieg `cron` na odtworzonych folderach, piętnaście przewidywań i piętnaście trafień (linia startu 692, `wc -l` 720, `(2331,3)`, zakładka 2360, Athena 777/spółkę, S3 155997 B/364 B, stróż `Up`, `git status --short` pusty). Zostaje, świadomie poza tym punktem: przepięcie wykresów, `ranking.py`, `test_plikow.py` i Power BI na Athenę |
| 7 | Test prawdziwej drogi | ✅ **26.09, z zastrzeżeniami** (jeden bieg z `cron`, w sobotę; gałąź „brak dzisiejszej świecy” pierwszy raz 28.09; opis na końcu wiersza). **Historia:** 🟨 **notatka zatwierdzona 22.09** [[Notatka-2026-09-22-test-prawdziwej-drogi]]: kształt wzorem `control.py` (funkcja zwraca listę problemów, osobna warstwa łączy się z Athena, tabela `gold_dane_dzienne`), cztery decyzje zatwierdzone (tabela, heurystyka dnia giełdowego pon–pt, uruchamianie na start ręcznie z laptopa, sprawdza dwa fakty z przeglądu i zgodność liczby dni między spółkami). **23.09 kod napisany przez Gracjana i sprawdzony na laptopie:** `kod/path.py` (czysta funkcja `sprawdz_daty(df, spolki, dzis)` z trzema sprawdzeniami + część `__main__` czytająca `SELECT spolka, data FROM gold_dane_dzienne`, data z `DROGA_DATA` albo `date.today()`), `kod/test_path.py` (6 testów na zmyślonych danych, `6 passed`). Pierwszy bieg na Athenie o ok. 19:20: `2334`, `Dane: OK` (przewidziane przed biegiem). Test alarmu na żywych danych, udawany dzień: 24.09 → trzy braki, 22.09 → trzy wpisy z przyszłości, bez zmiennej → `OK`; wszystkie trzy co do słowa zgodne z przewidywaniem. Instrukcja: [[Notatka-2026-09-23-jak-napisac-testy-drogi]]. Spełnione (a), (b), (e); **niespełnione (c) głośna awaria i (d) EC2**, bo uruchamiany ręcznie z laptopa (decyzja 3). **24–25.09 wpięcie do `control.py`, etap A (laptop) zamknięty:** notatka [[Notatka-2026-09-24-droga-na-ec2]] zatwierdzona 25.09 w całości (osiem decyzji w wariancie a: jedna lista i jedno zgłoszenie, problem z danymi = awaria u stróża, błąd Atheny jako problem, wspólna `pobierz_dane()`, jedna `KONTROLA_DATA`, czwarte sprawdzenie „spółka bez wierszy”, linia `Kontrola: Dane N wierszy`, ostrzeżenie `pandas` zostaje); instrukcja [[Notatka-2026-09-25-jak-wpiac-droge-do-kontroli]]. Kod Gracjana: `7 passed`, `18 passed` na obu plikach, bieg na bloku z 18.09 → 3 wpisy z przyszłości co do słowa, zmyślone klucze AWS → jeden problem `Athena: Błąd - … UnrecognizedClientException …` plus nieprzewidziany `Traceback` od `pyathena` na stderr (decyzja: zostawić). Commit `8b8f585`. **Dalej niespełnione (c) i (d)**, bo EC2 stoi na `6af7b48`. **26.09 etap B (EC2) zamknięty:** `git pull` do `fc6e262` (`13 files changed, 3258 insertions(+), 178 deletions(-)`, zgodnie z przewidywaniem), `git status --short` pusty; ręczne biegi bez `>>` (log 807 przed i po): E1 → `Kontrola: Dane 2340 wierszy`, `OK` (import `path.py` działa na Pythonie 3.9, rola EC2 czyta `gold/`), E2 → 4 problemy co do słowa, E3 → `Athena: Błąd - Execution failed on sql …` w 3 liniach (pandas 2.3.3 opakowuje błąd, przewidziane przed biegiem) + 25 linii od `pyathena`, E4 z prawdziwym stróżem → #8 `Failure` (`POST`, 2450 B = 154 + 2 + 2294), `up → down`, #9 `OK`, `down → up`. Szczegóły: [[Notatka-2026-09-24-droga-na-ec2]], „Wyniki etapu B”. **(c)** ✅ `Failure` u stróża i mail `DOWN` na Interii (16:19, 15 min po zmianie stanu; `UP` też 16:19). **(d)** ✅ pierwszy bieg z `cron` 26.09 o 18:30 (etap C) sprawdzony tego samego wieczoru: start w linii 808, `wc -l` 837, blok 30 linii zakończony `Kontrola: Dane 2340 wierszy` i `Kontrola: OK`, zakładka 2369, pliki 780 × 3, S3 156615 B / 338 B, zero błędów — wszystko przewidziane przed biegiem. **(e)** dziennik 26.09, CLAUDE.md. Zostaje obserwacja kilku dni (przewidywania do 02.10 w CLAUDE.md), przede wszystkim pierwszy dzień giełdowy 28.09 (blok 32 linie) |
| 10 | Dokumentacja | ⬜ w tle |
